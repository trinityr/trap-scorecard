...
router.post("/register", async (req: Request, res: Response) => {
  const { email, password, teamId, newTeamName } = req.body || {};
  ...
  try {
    ...
    const userResult = await client.query(
      `INSERT INTO users (email, password_hash, is_admin, team_id)
       VALUES ($1, $2, $3, $4)
       RETURNING id, email, is_admin, team_id`,
      [cleanEmail, passwordHash, isFirstUser, teamIdToUse]
    );
...
router.post("/login", async (req: Request, res: Response) => {
  ...
  try {
    const result = await pool.query(
      "SELECT id, email, password_hash, is_admin, team_id FROM users WHERE email = $1",
      [cleanEmail]
    );
